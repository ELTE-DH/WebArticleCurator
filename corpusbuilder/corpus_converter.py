...
def find_next_page_url(archive_page_raw_html, settings, archive_page_url_base, article_urls, page_num,
                       known_article_urls):
    """
    ...
    if settings['next_url_by_regex']:  # Method #1
        # We can not tell if the RE is bad, or we are on the last page...
        archive_page_next_page_url = extract_next_page_url(archive_page_raw_html, settings)
    elif settings['next_url_by_pagenum'] and len(article_urls) > 0 and \
            (len(known_article_urls) == 0 or
             # We allow some intersecting element as the archive may have been moved
             len(article_urls.minus(known_article_urls)) > settings['new_article_url_threshold']) \
